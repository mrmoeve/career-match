...
def open_beta_enabled() -> bool:
    raw = os.getenv("CAREER_MATCH_OPEN_BETA", "true").strip().lower()
...
def init_db() -> None:
    ...
    try:
        ...
        _add_column_if_missing(conn, "pdf_downloads", "user_id", "user_id INTEGER NOT NULL DEFAULT 0")
        _add_column_if_missing(conn, "pdf_downloads", "user_email", "user_email TEXT NOT NULL DEFAULT ''")
        _add_column_if_missing(conn, "pdf_downloads", "application_id", "application_id INTEGER NOT NULL DEFAULT 0")
        _add_column_if_missing(conn, "pdf_downloads", "tab_name", "tab_name TEXT NOT NULL DEFAULT ''")
        _add_column_if_missing(conn, "pdf_downloads", "file_name", "file_name TEXT NOT NULL DEFAULT ''")
...
def increment_free_assessment_usage(email: str) -> tuple[bool, str]:
    if open_beta_enabled():
        return True, "Open beta mode does not consume free assessment usage."
    normalized_email = _normalize_email(email)
...
def consume_assessment_credit(email: str) -> tuple[bool, str]:
    if open_beta_enabled():
        return True, "Open beta mode does not consume assessment credits."
    normalized_email = _normalize_email(email)
    access = get_assessment_access(normalized_email)
    if access["is_pro"]:
...
def get_dashboard_metrics(user_email: str) -> dict:
    ...
    try:
        ...
        pdf_downloads = int(_scalar(conn, "SELECT COUNT(*) FROM pdf_downloads WHERE user_email = ?", (normalized_email,), default=0) or 0)
        repeat_users = int(
            _scalar(
                conn,
                """
        ...
        return {
            ...
            "open_beta": access.get("open_beta", False),
            "pdf_downloads": pdf_downloads,
            "analyses_per_user": round((len(items) / 1), 1) if normalized_email else 0,
            "repeat_users": repeat_users,
            "feedback_submissions": int(_scalar(conn, "SELECT COUNT(*) FROM analysis_feedback WHERE user_email = ?", (normalized_email,), default=0) or 0),
        }
    finally:
        conn.close()
...
def save_pdf_download(record: PdfDownloadRecord) -> int:
    conn = _connect()
    try:
        download_id = _insert_and_get_id(
            conn,
            """
            INSERT INTO pdf_downloads (created_at, user_id, user_email, application_id, tab_name, file_name)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (
                record.created_at,
                int(record.user_id or 0),
                _normalize_email(record.user_email),
                int(record.application_id or 0),
                (record.tab_name or "").strip(),
                (record.file_name or "").strip(),
            ),
            "download_id",
        )
        conn.commit()
...
def admin_metrics() -> dict:
    ...
    try:
        ...
        totals = {
            ...
            "pdf_downloads": int(_scalar(conn, "SELECT COUNT(*) FROM pdf_downloads", default=0) or 0),
            ...
            "feedback_submissions": int(_scalar(conn, "SELECT COUNT(*) FROM analysis_feedback", default=0) or 0),
        ...
        totals["analyses_per_user"] = round((totals["total_analyses"] / total_users), 1) if total_users else 0.0
        totals["repeat_users"] = int(
            _scalar(
                conn,
                """
                SELECT COUNT(*)
        ...
        most_used_tabs = _fetchall_dicts(
            conn,
            """
            SELECT app_page, COUNT(*) AS view_count
            FROM page_views
            WHERE view_name = 'app' AND app_page <> ''
            GROUP BY app_page
            ORDER BY view_count DESC, app_page ASC
            LIMIT 10
            """,
        )
        return {
